// HMAC-signed callbacks back to the complete-carousel-render edge function.
// The edge function validates the signature with the same RENDER_WORKER_SECRET
// and updates the carousel_slides + carousel_render_jobs rows accordingly.
...
export async function callback(body: CallbackBody): Promise<void> {
  ...
  const res = await fetch(`${url}/complete-carousel-render`, {
    method: "POST",
    headers: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`callback_${res.status}: ${text.slice(0, 300)}`);
  }
}
