...
| `/content-center` | Sub-landing de producto "Content Center" (showcase de features + lista de espera). No linkeada desde la home; solo acceso por ruta directa | public |
| `/content-center/features` | Catálogo exhaustivo de features del producto (enlace secundario desde `/content-center`) | public |
...
| `content_center_waitlist` | M20 | **Lista de espera de la sub-landing de producto `/content-center`.** `content_center_waitlist` (`email`, `name`, `platforms text[]` = instagram/youtube/tiktok, `source`, `user_agent`, `notified`, timestamps). Unique index en `lower(email)` (idempotencia). RLS: **solo admin lee** (`has_role(auth.uid(),'admin')`); no hay policy de INSERT para `anon` — la escritura va exclusivamente por la edge function `join-waitlist` (service role). Trigger `set_updated_at`. |
...
| `join-waitlist` | **NO (público)** | (M20) Endpoint público de la lista de espera de `/content-center`. Recibe `{ email, name?, platforms?: string[], website? }`. `website` es honeypot anti-bot (si viene relleno → `{ok:true}` sin hacer nada). Valida formato de email, hace upsert idempotente en `content_center_waitlist` con **service role** (no reenvía email si el email ya existía → `{ok:true, already:true}`), y manda email de bienvenida brandeado vía Resend (confirma alta, promete avisar cuando se abra al público + novedades, **sin prometer fechas**). Reusa `RESEND_API_KEY` / `FROM_EMAIL` / `APP_URL` (sin secrets nuevos). Si Resend falla, el alta igual queda guardada (`{ok:true, emailed:false}`). |
