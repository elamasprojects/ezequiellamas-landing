...
| ~~`videos-final`~~ | private | `{owner_id}/{uuid}.{ext}` | M10 — **Deprecated post-M12**. Reemplazado por Bunny Stream. Bucket vacío. Borrarlo manualmente del dashboard de Supabase cuando se quiera limpiar. |
...
| `bunny-create-video` | yes | (M12) Recibe `{ filename, title? }`. Crea un video vacío en Bunny Stream library vía `POST https://video.bunnycdn.com/library/{LIB}/videos`. Genera firma TUS (sha256 de `lib_id + api_key + expiration + video_id`) con TTL 1h. Devuelve `{ video_id, library_id, upload_url, auth_signature, auth_expiration_time, cdn_url }` para que el cliente use `tus-js-client` directo a Bunny. |
| `transcribe-bunny-video` | yes (o service) | (M13) Recibe `{ scheduled_post_id?, bunny_video_id?, language?, force? }`. Si `scheduled_post_id` y `transcript_status='done'` → devuelve cached. Si no, hace `HEAD` a `https://{BUNNY_CDN_HOSTNAME}/{video_id}/original`. Si >25MB → `transcript_status='too_large'` + 422. Si OK, descarga el blob, POST multipart a OpenAI Whisper-1 con `response_format=verbose_json`, cachea en `scheduled_posts.transcript / transcript_language / transcript_status='done'`. Devuelve `{ transcript, language, duration_seconds, cached }`. |
| `generate-captions` | yes (o service) | (M13) Recibe `{ scheduled_post_id?, bunny_video_id?, transcript?, platforms[], format_id?, force_regenerate? }`. Si no llega transcript inline ni cache, invoca internamente `transcribe-bunny-video`. Si `format_id` seteado, lee `formats(name, description)` y lo inyecta en el system prompt. Llama Claude Sonnet 4.6 con tool `emit_captions` (input_schema con `caption_default, captions{ig,yt,tt}, youtube_title, hashtags[]`). Sanitiza hashtags (lowercase, alfanuméricos), trunca title a 100 chars, filtra captions a las plataformas pedidas. Devuelve `{ caption_default, captions, youtube_title, hashtags, used_format }`. |
...
2.5. **Bunny Stream**: crear cuenta en bunny.net → Stream → Create Library. Setear `BUNNY_LIBRARY_ID` (numérico), `BUNNY_LIBRARY_KEY` (API key del library) y `BUNNY_CDN_HOSTNAME` (pull zone, ej `vz-xxxxx.b-cdn.net`) en Supabase secrets. Bunny encodea automáticamente y expone el raw MP4 en `https://{cdn}/{video_id}/original` que es la URL pública que pasamos a Zernio.
