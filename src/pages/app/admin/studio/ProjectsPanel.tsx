...
/**
 * "Proyectos" tab of the YouTube hub. Projects list is primary; capturing a new
 * long-form idea is a secondary "Agregar idea" action (auto-opened via `?new=1`,
 * the destination of the global "Nueva idea → Largo" modal).
 */
export default function ProjectsPanel() {
  ...
  const [showCapture, setShowCapture] = useState(params.get("new") === "1");
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <p className="max-w-xl text-sm" style={{ color: "var(--ll-text-muted)" }}>
          Tus videos largos. Contá la idea y la IA arma la estructura (intro, capítulos, cierre) con
          tiempos; después editás cada sección.
        </p>
        {!showCapture && (
          <Button variant="brand" onClick={() => setShowCapture(true)}>
            <Plus className="h-4 w-4" /> Agregar idea
          </Button>
        )}
      </div>
          ...
          <div className="flex justify-end">
            <Button variant="brand" onClick={() => generate.mutate()} disabled={generate.isPending || !idea.trim()}>
              {generate.isPending ? (
                ...
                <>
                  <Sparkles className="h-4 w-4" /> Generar estructura
                </>
              )}
            </Button>
          </div>
      ...
      <section className="space-y-3">
        ...
        {isLoading ? (
          ...
          <p className="text-sm" style={{ color: "var(--ll-text-muted)" }}>
            Todavía no creaste ningún video largo.
          </p>
        ) : (
          <ul className="space-y-2">
            {projects.map((p) => (
              <li
                key={p.id}
                className="group flex items-center gap-3 rounded-lg border border-[var(--ll-border)] bg-[var(--ll-surface)] p-4"
              >
                ...
                <Button
                  ...
                  onClick={() => {
                    if (confirm("¿Borrar este proyecto?")) del.mutate(p.id);
                  }}
                  aria-label="Borrar"
                >
